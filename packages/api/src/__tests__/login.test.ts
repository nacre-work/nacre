...
/**
 * The tokens out of a sign-in, or `undefined`.
 *
 * `login` answers a union since the second factor landed: a session, or a
 * challenge for the half that is still missing. Every case in this file is
 * about an account with no factor, so unwrapping here keeps them saying what
 * they were written to say — and the one case that would silently pass on the
 * wrong branch is the reason this asserts the kind rather than casting.
 */
const tokensOf = (outcome: Awaited<ReturnType<Login['login']>>) => {
  if (outcome === undefined) return undefined
  if (outcome.kind !== 'tokens') throw new Error(`expected a session, got ${outcome.kind}`)
...
when('signing in', () => {
  ...
  it('issues a token whose organization comes from the row, not the request', async () => {
    const tokens = tokensOf(await login.login({ email: 'alice@login.test', password: PASSWORD }))
  ...
  it('refuses a password that belongs to another organization’s account', async () => {
    ...
    expect(tokensOf(await login.login({ email: 'bob@login.test', password: PASSWORD, organization: 'login-a' })))
      .toBeUndefined()

    const proper = tokensOf(await login.login({ email: 'bob@login.test', password: PASSWORD, organization: 'login-b' }))
  ...
  it('refuses an address that exists in more than one organization, without saying so', async () => {
    ...
    // appears in.
    expect(tokensOf(await login.login({ email: 'shared@login.test', password: PASSWORD }))).toBeUndefined()

    // Named explicitly, it works — the ambiguity was the problem, not the user.
    expect(
      tokensOf(await login.login({ email: 'shared@login.test', password: PASSWORD, organization: 'login-a' })),
    ).toBeDefined()
  })

  it('refuses a wrong password, an unknown address, no password set, and a disabled account alike', async () => {
    expect(tokensOf(await login.login({ email: 'alice@login.test', password: 'wrong' }))).toBeUndefined()
    expect(tokensOf(await login.login({ email: 'nobody@login.test', password: PASSWORD }))).toBeUndefined()
    // An account with no password must not be an account anyone can sign into.
    expect(tokensOf(await login.login({ email: 'nopass@login.test', password: '' }))).toBeUndefined()
    expect(tokensOf(await login.login({ email: 'off@login.test', password: PASSWORD }))).toBeUndefined()
  })

  it('is case- and whitespace-insensitive on the address', async () => {
    expect(tokensOf(await login.login({ email: '  Alice@Login.Test ', password: PASSWORD }))).toBeDefined()
  })
  ...
  it('refuses an expired refresh token', async () => {
    ...
    const tokens = tokensOf(await past.login({ email: 'alice@login.test', password: PASSWORD }))
  ...
  it('a fresh login is a new family, so signing out one device keeps the others', async () => {
    const laptop = tokensOf(await login.login({ email: 'alice@login.test', password: PASSWORD }))
    const phone = tokensOf(await login.login({ email: 'alice@login.test', password: PASSWORD }))
